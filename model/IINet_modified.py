...
class Net(nn.Module):
    def __init__(self, args):
        ...
        self.factor = factor
        self.angRes = angRes
        self.down = DownBlock(self.factor)
        self.IntraFeaExtract = FeaExtract(channel,self.factor)
        self.InterFeaExtract = Extract_inter_fea(channel, angRes,self.factor)
    ...
    def forward(self, x):
        xxx = x
        x = stack2jigsaw(x, self.angRes, self.factor)
        ...
        out_sv = self.UpSample(intra_fea) + xxx.unsqueeze(2)
        out = FormOutput(out_sv)
        out = jigsaw2stack(out, self.angRes)

        return out

def stack2jigsaw(x, angRes, scale):
    """
    ...
    :return: [N, 1, UX, VY]
    """
    # # torch.Size([4, 25, 64, 64])
    # n, uv, h, w = x.size()

    # x = x.view(n, c, h//scale, scale, w//scale, scale)
    # x = x.permute(0, 3, 5, 1, 2, 4).contiguous()
    # x = x.view(n, c * (scale**2), h//scale, w//scale)


    N, X, Y = x.shape[0], x.shape[2], x.shape[3]
            #  0    1      2        3        4       5        6
    x = x.view(N, angRes, angRes, X//scale, scale, Y//scale, scale) # [N, U, V, X, s, Y, s]
    x = x.permute(0, 4, 6, 1, 2, 3, 5).contiguous() #[N, s, s, U, V, X, Y]
    x = x.permute(0, 1, 2, 3, 5, 4, 6).contiguous() # [N, s, s, U, X, V, Y]
    x = x.view(N, scale**2, angRes * X//scale, angRes * Y//scale) # [N,ss, UX, VY]
...
class FeaExtract(nn.Module):
    def __init__(self, channel, factor):
        ...
        self.factor = factor
        self.FEconv = nn.Conv2d(self.factor**2, channel, kernel_size=1, stride=1, padding=0, bias=False)
    ...
    def forward(self, x_mv):
        ...
        x_mv = x_mv.contiguous().view(b * n, r, h, w)
        ...
        intra_fea = intra_fea.unsqueeze(1).contiguous().view(b, -1, c, h, w)  
                        # .permute(0,2,1,3,4)  # intra_fea:  B, N, C, H, W
        return intra_fea


class Extract_inter_fea(nn.Module):
    def __init__(self, channel, angRes, factor):
        super(Extract_inter_fea, self).__init__()
        self.factor = factor
        self.FEconv = nn.Conv2d(self.factor**2 *angRes * angRes, channel, kernel_size=1, stride=1, padding=0, bias=False)
        self.FERB_1 = ResASPP(channel)
        self.FERB_2 = RB(channel)
        self.FERB_3 = ResASPP(channel)
        self.FERB_4 = RB(channel)
