...
class Net(nn.Module):
    def __init__(self, args):
        ...
        self.AngFE = nn.Sequential(
            DownBlock(self.factor),
            nn.Conv2d(self.factor**2, channels, kernel_size=int(self.angRes), stride=int(self.angRes), padding=0, bias=False)
            )
        self.SpaFE = nn.Sequential(
            DownBlock(self.factor),
            nn.Conv2d(self.factor**2, channels, kernel_size=3, stride=1, dilation=int(self.angRes), padding=int(self.angRes), bias=False)
            )
